class RoomsController < ApplicationController
  ...
  def room_other_params
    params.require(:room_tag_user_form).permit(:title,:tag_name).merge(deadline: Time.now + 3.days, user_id: current_user.id, user_name: current_user.nickname, owner_id: current_user.id)
  end
